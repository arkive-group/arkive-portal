...
export default function FulfillPopup({ fulfillmentOrder, open, setOpen }) {
    ...
    const [fulfillmentOrderIds, setFulfillmentOrderIds] = useState(fulfillmentOrder.fulfillmentOrderIds ?? []);
    const [trackingNumbers, setTrackingNumbers] = useState(Array(fulfillmentOrderIds.length));
    const [trackingUrls, setTrackingUrls] = useState(Array(fulfillmentOrderIds.length));
    const [trackingCompanies, setTrackingCompanies] = useState(Array(fulfillmentOrderIds.length));
    ...
    const handleFulfillOrder = async (e) => {
        ...
        for (let i = 0; i < fulfillmentOrderIds.length; i++) {
            ...
            if (response.userErrors !== undefined && response.userErrors.length > 0) {
                enqueueSnackbar(`Error fulfilling order with ID: ${fulfillmentOrderIds[i]}: ${response.userErrors[0]?.message}`, { variant: "error" });
